...
extension WeatherData {
	...
	static func makeDefaultValue() -> WeatherData {
		return  WeatherData(coord: Coord(lon: 126.93407, lat: 37.38575)
	...
	static let `default` = WeatherData(coord: Coord(lon: 126.93407, lat: 37.38575)
									   , weather: [Weather(id: 0, main: "Unknown", description: "unknown", icon: "unknown")]
									   , base: "unknown"
									   , main: Main(temp: 0.0, feels_like: 0.0, pressure: 0, humidity: 0, temp_min: 0.0, temp_max: 0.0)
									   , visibility: 0
									   , wind: Wind(speed: 0.0, deg: 0)
...
extension Entity_Current_Weather {
	...
	var toWeatherData: WeatherData {
		...
		let weathers: [WeatherData.Weather] = [WeatherData.Weather(
			id: 0,
			main: self.main!,
		...
		let clouds = WeatherData.Clouds( all: Int(self.all)
		)
